...
6.
a, b - любое выражение
Func1, Func2 - функции, которые вызовутся для вычисления условия
...
> Еще одной хорошей практикой является следующее:<br/>
<br/>
Пусть у нас есть цикл<br/>
<br/>
...
<br/>
у нас на каждой итерации цикла вычисляется длина строки. Это плохо (за исключением когда строка в теле цикла меняется — но его мы рассматривать не будем). Лучше написать следующим образом:<br/>
<br/>
<pre class="prettyprint"><code>
...
>> Подсказали хороший пример, как сделать код быстрее:<br/>
<pre>
for (int i = 0; str[i]; ++i)
{
}</pre>
...
>> Мне тут подсказали такой вариант:<br/>
for (int i = 0; str[i]; ++i){}<br/>
<br/>
Предполагалось, что он будет еще быстрее, но это не так.<br/>
<br/>
Вариант for ( int i = 0, size = strlen( string ); i &lt; size; ++i ) будет в каждом витке цикла сравнивать две целые стековые переменные, а вариант for (int i = 0; str[i]; ++i) будет вычислять текущий указатель, разыменовывать его получая по нему данные, брать от них только 1 байт и только потом сравнивать его с нулем. Это будет дольше, чем просто сравнение.

> **Saturn812, 14 мая 2011, 22:34**
> Многие из этих вещей (особенно примитивные ++i vs i++) компилятор уже давно научился оптимизировать сам.<br/>
А так вообще полезно для общего сведения.

>> **FiloXSee, 17 мая 2011, 10:34**
>> Ты не совсем прав. Действительно в теле цикла разницы между вариантами<br/>
for( int i = 0; i &lt; 10; i++ )
